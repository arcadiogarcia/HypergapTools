    ...
    function realparse(xmlDoc) {
        ...
        for (var k = 0; k < xmlDoc.getElementsByTagName("spritesheet").length; k++) {
            ...
            if (thisspritesheet.getAttributeNode("src") != undefined) {
                ...
                if(rootFolder){
                    newspritesheet.img.src = rootFolder+(thisspritesheet.getAttributeNode("src").value);
                }else{
        ...
        xmlhttp.onreadystatechange = function () {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                if(xmlhttp.responseXML==null){
                    parser(( new window.DOMParser() ).parseFromString(xmlhttp.responseText, "text/xml"));
                    //Fallback for when the payload is not recognized as XML (electron), maybe because of mising MIME?
                }else{
